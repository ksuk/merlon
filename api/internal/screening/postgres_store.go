...
func (s *PostgresFailureTracker) RecordSuccess(ctx context.Context, id string) error {
	_, err := s.pool.Exec(ctx, `INSERT INTO screening_list_failures(list_id,consecutive_failures,last_success_at) VALUES($1,0,now()) ON CONFLICT(list_id) DO UPDATE SET consecutive_failures=0,last_success_at=now()`, id)
	return err
}
func (s *PostgresFailureTracker) RecordFailure(ctx context.Context, id string) (int, error) {
	var n int
	err := s.pool.QueryRow(ctx, `INSERT INTO screening_list_failures(list_id,consecutive_failures) VALUES($1,1) ON CONFLICT(list_id) DO UPDATE SET consecutive_failures=screening_list_failures.consecutive_failures+1 RETURNING consecutive_failures`, id).Scan(&n)
	return n, err
}
