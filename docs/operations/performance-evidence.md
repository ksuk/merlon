...
3. Complete setup and create a temporary Analyst API key. Keep the value only
...
4. Record host CPU, memory, OS, Docker version, image digest, PostgreSQL
...
- the target version, exact commit, build timestamp when available,
  authentication mode, and base currency;
