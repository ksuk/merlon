...
`outcome-matcher-v1` contract evaluates baseline, candidate, and delta alerts
...
`comparison/known_matter_coverage` job. The worker builds a deterministic union
...
snapshot, and assumptions alongside the rate.
