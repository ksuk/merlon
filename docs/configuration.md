...
| `MERLON_BUILD_VERSION` | `dev` | Demo Compose build version embedded in the System status and image metadata. |
| `MERLON_BUILD_REVISION` | unset | Full source commit embedded in the Demo image; release acceptance must provide the exact 40-character SHA. |
| `MERLON_BUILD_BUILT_AT` | unset | UTC build timestamp embedded in the Demo image. |
