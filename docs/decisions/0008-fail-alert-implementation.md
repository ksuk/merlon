...
- 数値（3秒/2回/30秒/Half-Open 1リクエスト）は overview.md §4.4 障害対応マトリクスの Rust エンジン行に明記された値をそのまま採用する。実装判断ではなく仕様の転記であるため独自の調整は行わない
...
- **サーキットブレーカーライブラリ（gobreaker 等）の導入** — 状態遷移ロジック自体は小さく、テスト容易性のためのクロック注入（`nowFn`）など独自要件があったため、外部依存を増やさず自前実装とした（03_implementation-plan.md の「外部ライブラリ追加は最小限に」方針に従う）
