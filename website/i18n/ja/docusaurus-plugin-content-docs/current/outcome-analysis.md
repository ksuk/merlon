---
sidebar_position: 10
title: バックテストのアウトカム分析と既知事案カバレッジ
---

# バックテストのアウトカム分析と既知事案カバレッジ

バックテストジョブには `outcome_analysis` を加算形式で返します。共有
`outcome-matcher-v2` 契約が、ベースライン・候補・差分のアラートを、過去
...
`comparison/known_matter_coverage` の永続ジョブをキューへ追加し、半開区間の
リプレイ期間（`from`、`to`）を必須とします。ワーカーは `snapshot_at` 時点で
参照可能な取引に固定ルールセットをリプレイしてから、
...
では分母、matcher 版、リプレイ期間、ルールセット、スナップショット、前提を常に表示します。
