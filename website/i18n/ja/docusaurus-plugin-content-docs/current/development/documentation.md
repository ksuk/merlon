...
英語ファイルに未コミットのローカル変更がある場合はピン留めできるコミットハッシュが存在しないため、代わりに `sha256:` プレフィックス付きのコンテンツハッシュ ack を使用する:

```json
{ "getting-started.md": "sha256:<en ファイル内容の sha256 hex ダイジェスト>" }
```

**陳腐化エントリの規則**: ハッシュが英語ファイルの現在の HEAD ハッシュ（`sha256:` ack の場合は現在のファイル内容）と一致しない ack はチェックを失敗させる。以降の英語側の変更のたびに再確認する（または翻訳を更新する）必要がある。
