...
1. タグが厳密な SemVer であること（`-alpha.N`／`-beta.N`／`-rc.N` のプレリリース識別子は許容）。
...
| `release-manifest.json` | タグ、コミット、イメージ、ダイジェスト、SBOM ハッシュ、来歴 URL |
...
| アクティブなメンテナが1名 | `MAINTAINERS.md` に明記。本番リリースはこの解消をゲートとしている |
