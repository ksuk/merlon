...
本プロジェクトのメンテナは1名である。したがって `vX.Y.Z` イメージが主張するのは、リリースコミット上で全自動ゲートが通過したことと、そこに含まれる変更にセルフレビュー証跡が存在することである。独立した承認と職務分離は**主張しない**。1人が自分の作業を独立してレビューすることはできないためであり、その事実はイメージ自身が述べる。
