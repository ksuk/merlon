...
Merlon は、ツールチェーン、ベースイメージ、GitHub Actions、セキュリティ検査ツールを、レビュー済みバージョンまたは不変ダイジェストに固定する。固定は再現可能な証跡であり、更新を停止する理由ではない。本台帳の最終確認日は **2026年7月26日** である。

## ランタイム台帳

| コンポーネント | リポジトリの固定値 | 上流サポート期限 | 必要な対応 |
|---|---|---|---|
| Go | 1.26.5 | [新しいメジャー版が2つ出るまでサポート](https://go.dev/doc/devel/release)されるため、1.26 は 1.28 のリリース時にサポート終了 | セキュリティ版へ直ちに更新し、1.28 のリリース前に 1.27 への移行を完了する。 |
