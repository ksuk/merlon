...
新規デプロイにはアカウントが1つも存在しない。すなわち誰もログインできず、通常の経路でユーザーを作成することもできない。ブラウザからは `/setup` で到達する `POST /api/v1/setup` がこれを解決する。このエンドポイントは最初の Admin アカウントを作成し、それ以外は何もしない。
...
以降のアカウントはすべて、既存の Admin が「ユーザ管理」から、後述のロールモデルに従って作成する。
