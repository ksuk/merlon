...
パスワードは12文字以上。作成されるアカウントは Admin である。そのAdminがログインした後は、「ユーザ管理」で後続アカウントの作成、ロール、有効状態、代替パスワードを管理する。対応APIは `POST /api/v1/admin/users` から始まる。
...
別のAdminがログインできる場合は、「ユーザ管理」で代替パスワードを設定する。この変更は対象アカウントの既存セッションを失効させる。Adminが誰もログインできない場合は、deploymentのdatabase backupとincident手順でアクセスを復旧する。`/setup`から2人目の「最初の管理者」は作成できない。
...
連続失敗はリストごとにカウントされ、3回連続で運用者向けにその失敗がフラグとして示される。インポートジョブは `needs_operational_alert=true` を含む構造化エラーログを出力し、ダッシュボードのスクリーニング鮮度データにも同じフラグが載り、`merlon_screening_list_stale_days` ゲージが各リストの経過日数を `/metrics` で公開する。Merlon 自身が誰かに通知することはない。デプロイ環境側にこれらを人へ届ける仕組みがなければ、古さは気づかれないまま残る。それは自組織の監視の性質であって、Merlon が単独で行うことではない。

スクリーニング結果が想定より古いと感じた場合は、照合ロジックではなく当該リストのインポートジョブを確認すること。結果は誤っているのではなく、古い。
